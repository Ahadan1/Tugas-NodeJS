    ...
    try {
                    ...
                    include: [
                        {
                            model: Product,
                            as: 'product',
                            attributes: ['name', 'description', 'stock'],
                            include: [
                                {
                                    model: Category,
                                    as: 'category',
        ...
        const formattedTransactions = transactions.map(transaction => ({
            ...
            admin: transaction.admin ? {
                firstName: transaction.admin.firstName,
                lastName: transaction.admin.lastName,
...
const getProductTransactionHistory = async (req, res) => {
    try {
        ...
        console.log('Getting transaction history for product:', productId);
        ...
        // Find all transaction items for this product
        const { count, rows: transactionItems } = await TransactionItem.findAndCountAll({
            where: { productId },
            include: [{
                model: Transaction,
                as: 'transaction',
        ...
        // Format the response
        const formattedHistory = transactionItems.map(item => ({
            id: item.id,
            ...
            stockBefore: item.stockBefore,
            stockAfter: item.stockAfter,
            processedBy: `${item.transaction.admin.firstName} ${item.transaction.admin.lastName}`,
